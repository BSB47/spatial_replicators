...
int main(int argc, char **argv) {
  ...
  case 0:
    ...
    break;
  default:
    std::cerr << "main(): Unknown simulation_type" << std::endl;
    break;
  }
...
void core_simulation(MyCA &myca, const long Time) {
  ...
  if (Para::model_type == Para::VESIC_NEUT_EQUIL) {
    ...
    std::cerr << "main.c core_simulation() Do not use "
                 "PARALLEL_WELL_MIXED_EQUIL_NO_COMPLEX"
              << std::endl;
    exit(-1);
    myca.parallel_update_replicator_whole_no_complex_EQA();
  } else if (Para::model_type == Para::PARALLEL_WELL_MIXED_EQUIL) {
    std::cerr << "main.c core_simulation() Do not use PARALLEL_WELL_MIXED_EQUIL"
              << std::endl;
    exit(-1);
    myca.parallel_update_replicator_whole_EQA();
    ...
    std::cerr << "main() Error, unknown model_type=" << Para::model_type
              << std::endl;
    exit(-1);
  }
}
...
void single_run() {
  ...
  for (Time = init_time; Time <= Para::max_time; ++Time) {
    ...
    if (is_end || is_usr1 ||
        (Time % Para::save_interval == 0 && Time != init_time)) {
...
void scale_rates() {
  ...
  Para::vesicle_update_interval = static_cast<int>(
      0.5 + Para::vesicle_update_interval_d / Para::rate_scale);
  if (Para::vesicle_update_interval < 1) {
    Para::vesicle_update_interval = 1;
    std::cerr << "scale_rates() Warning, rate_scale is too small so that "
                 "vesicle_update_interval had to be set to 1."
              << std::endl;
  }
  ...
  if (Para::model_type == Para::VESIC_NEUT_EQUIL ||
    ...
    if (Para::max_fold_unfold_rate != 1.) {
      std::cerr << "scale_rates() Although the system is under the equilibrium "
                   "condition, max_fold_unfold_rate is not set to 1. Abort."
                << std::endl;
      exit(-1);
    }
  } else if (Para::model_type == Para::PARALLEL_ONLY_CPM ||
    ...
    std::cerr << "scale_rates() Unkown model_type. Do I have to scale "
                 "init_fold_rate_repli and init_fold_rate_para?"
              << std::endl;
    exit(-1);
  }
  ...
  /* Mutations are mutually exclusive. The sum of mutation rates must be
    less than one. */
  if (1. < Para::mut_rate_rna_dna_repl_rate + 2. * Para::mut_rate_recog_param +
               Para::mut_rate_to_junk + Para::mut_rate_to_para ||
      1. < Para::mut_rate_lipid_synth_rate + Para::mut_rate_fold_unfold_rate +
               Para::mut_rate_to_junk) {
    std::cerr << "scale_rates() The sum of mutation rates is not less than 1!"
              << std::endl;
    exit(-1);
  }
  ...
  if ((Para::model_type == Para::WELL_MIXED_EQUIL ||
    ...
    std::cerr << "scale_rates() If the system is well-mixed, diffusion_rate "
                 "must be 0."
              << std::endl;
    exit(-1);
  }
  ...
  if (Para::model_type == Para::SURFACE_EQUIL_NO_COMPLEX ||
    ...
    if (Para::max_asso_diss_rate != 0.) {
      std::cerr << "scale_rates() max_asso_diss_rate != 0 even though the "
                   "model assumes complex formation."
                << std::endl;
      exit(-1);
    }
    ...
    if (Para::mut_rate_to_junk != 0 || Para::mut_rate_to_para != 0) {
      std::cerr << "scale_rates() mut_rate_junk or mut_rate_para is not zero. "
                   "If you are intending to do simulations that require "
                   "mutations to junk molecules or parasites, comment out "
                   "appropriate regiouns in Molecule::copy_with_mutation()."
                << std::endl;
      exit(-1);
    }
  }
  ...
  if (Para::rna_dna_repl_constrain == 0 || Para::rna_dna_repl_constrain == 1) {
    max_repli_rate = (Para::max_rna_repli_rate > Para::max_dna_repli_rate)
                         ? Para::max_rna_repli_rate
                         : Para::max_dna_repli_rate;
  } else {
  ...
  if (Para::model_type == Para::VESIC_NEUT_EQUIL ||
    ...
    if (Para::fold_unfold_constrain == 0) {
      std::cerr << "EMPTY " << max_repli_rate + Para::diffusion_rate
                << std::endl;
      /* As I wrote before, the complex association between X and X has
         ...
         max_asso_diss_rate. */
      std::cerr << "REPLI_RNA "
                << Para::max_asso_diss_rate + Para::diffusion_rate +
                       Para::rna_decay_rate
                << std::endl;
      ...
      double max_paras_reaction =
          (Para::max_asso_diss_rate > Para::max_lipid_synth_rate)
              ? Para::max_asso_diss_rate
              : Para::max_lipid_synth_rate;
      std::cerr << "PARAS_RNA "
                << max_paras_reaction + Para::diffusion_rate +
                       Para::rna_decay_rate
                << std::endl;
      std::cerr << "DNA "
                << Para::max_asso_diss_rate + Para::diffusion_rate +
                       Para::dna_decay_rate
                << std::endl;
      std::cerr << "COMPLEX (RNA/DNA) "
                << max_repli_rate + Para::max_asso_diss_rate +
                       Para::diffusion_rate_complex +
                       ((Para::rna_decay_rate > Para::dna_decay_rate)
                            ? Para::rna_decay_rate
                            : Para::dna_decay_rate)
                << std::endl;
      ...
      /* As I wrote before, the complex association between X and X has
         ...
         REPLI_RNA_1 + REPLI_RNA_2:
         bind_param*(rna_recog_param_1+rna_recog_param_2) REPLI_RNA_1 +
         REPLI_DNA_2: bind_param*dna_recog_param_1 REPLI_RNA_1 + PARAS_RNA_2:
         parasite_advantage*bind_param*rna_recog_param_1 REPLI_RNA_1 +
         PARAS_DNA_2: parasite_advantage*bind_param*dna_recog_param_1
         ...
         rate. */
      std::cerr << "REPLI_RNA "
                << (2. > Para::parasite_advantage
                        ? 2. * Para::max_asso_diss_rate
                        : Para::parasite_advantage * Para::max_asso_diss_rate) +
                       Para::diffusion_rate + Para::rna_decay_rate
                << std::endl;
      ...
      std::cerr << "PARAS_RNA "
                << Para::parasite_advantage * Para::max_asso_diss_rate +
                       Para::max_lipid_synth_rate + Para::diffusion_rate +
                       Para::rna_decay_rate
                << std::endl;
      std::cerr << "PARAS_DNA "
                << Para::parasite_advantage * Para::max_asso_diss_rate +
                       Para::diffusion_rate + Para::dna_decay_rate
                << std::endl;
      ...
      std::cerr << "scale_rates(): Unknown value in "
                   "switch_mol_recognition_assumption."
                << std::endl;
      exit(-1);
    }
    ...
    /* For a system without complex formation, the rate of replication
       ...
       m*k. */
    std::cerr << "EMPTY "
              << Para::max_asso_diss_rate * max_repli_rate +
                     Para::diffusion_rate
              << std::endl;
    std::cerr << "REPLI_RNA "
              << Para::max_asso_diss_rate * max_repli_rate +
    ...
    double max_paras_reaction =
        (Para::max_asso_diss_rate * max_repli_rate > Para::max_lipid_synth_rate)
            ? Para::max_asso_diss_rate * max_repli_rate
            : Para::max_lipid_synth_rate;
    std::cerr << "PARAS_RNA "
              << max_paras_reaction + Para::diffusion_rate +
...
void args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (argument == "-Display") {
      i++;
      if (i == argc) {
        std::cerr << "No parameter after " << argument << std::endl;
        exit(-1);
      }
    ...
    /* If Continue=1, the program looks for the last ".sav" or
      ...
      std::cerr
          << "-SaveInt -Scale -Display -DisplayInt -Movie -MovieInt -MovieDir "
             "-Seed -MaxTime -RateScale -DiffRate -CrossDiff -RNADecayRate "
             "-DNADecayRate -MaxRNARepliRate -MaxDNARepliRate "
             "-MaxFoldUnfoldRate -MaxLipidSynthRate -InitRNApFoldRate "
             "-InitDNApFoldRate -InitParaFoldRate -InitLipidSynthRate "
             "-MutRateRecogParam -MutStepRecogParam -MutRateRNADNAReplRate "
             "-MutRateFoldUnfoldRate -MutStepFoldUnfoldRate "
             "-MutRateLipidSynthRate -MutStepLipidSynthRate -MutRateToPara "
             "-MutRateToJunk -InitRNApRNArecog -InitRNApDNArecog "
             "-InitDNApRNArecog -InitDNApDNArecog -InitRNApRNAdens "
             "-InitRNApDNAdens -InitDNApRNAdens -InitDNApDNAdens "
             "-InitParaRNAdens -InitParaDNAdens -VolThr -NeutralGrowthFactor "
             "-FixedTarVol -VesDecayRate -MinRNApFoldRate -PlotInt -PlotFile "
             "-ReadFile -ReadFile2 -OldRateScale -HistMaxY -HistRecoMinX "
             "-HistRecoMaxX -HistFoldMinX -HistFoldMaxX -CompetitionExp "
             "-FileNrow -FileNcol -VesicleFile -Continue -RemoveSpecies "
             "-CountRNADNAInt -ParasiteAdv -MutInitRNARecogRp "
             "-MutInitDNARecogRp -MutInitRNARecogDp -MutInitDNARecogDp "
             "-MutInitFoldRate -MutRateRNARecogParamRp -MutRateDNARecogParamRp "
             "-MutRateFoldUnfoldRateRp -MutRateRNARecogParamDp "
             "-MutRateDNARecogParamDp -MutRateFoldUnfoldRateDp"
          << std::endl;
      exit(-1);
    }
  }
...
void button2(int x, int y) { myca_p->button2(x, y); }

void button3(int x, int y) { myca_p->button3(x, y); }
...
long setup_continuation() {
  if (system(
          "$HOME/study/dna-simple/program/find_last_save/find_last_save.sh")) {
    std::cerr
        << "setup_continuation() Error occured while calling find_last_save.sh"
        << std::endl;
    exit(-1);
  }
  ...
  if (!fin.is_open()) {
    std::cerr << "MyCA::setup_continuation() Error. Cannot open last_save."
              << std::endl;
    exit(-1);
  }
